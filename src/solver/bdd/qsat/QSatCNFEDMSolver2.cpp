...
namespace solver {
    namespace bdd {
        namespace qsat {
            ...
            QSatCNFEDMSolver2::QSatCNFEDMSolver2(const Application& app, bool checkUnsat)
            : ::Solver(app)
            , checkUnsat(checkUnsat) {
            }

            Computation* QSatCNFEDMSolver2::compute(htd::vertex_t currentNode) {
                ...
                if (decomposition->isLeaf(currentNode)) {
                    ...
                    // Do introduction
                    app.getPrinter().solverIntermediateEvent(currentNode, *cC, "introducing clauses");
                    // TODO: Only consider introduced vertices
                    BDD currentClauses = this->currentClauses(currentNode);
                    nsfMan.apply(*cC, [&currentClauses](BDD bdd) -> BDD {
                        return bdd *= currentClauses;
                    });
                    app.getPrinter().solverIntermediateEvent(currentNode, *cC, "introducing clauses - done");
                    app.getPrinter().solverIntermediateEvent(currentNode, *cC, "optimizing");
                    nsfMan.optimize(*cC);
                    app.getPrinter().solverIntermediateEvent(currentNode, *cC, "optimizing - done");
            ...
            const std::vector<BDD> QSatCNFEDMSolver2::getCubesAtLevels(htd::vertex_t currentNode) const {
            ...
            BDD QSatCNFEDMSolver2::currentClauses(htd::vertex_t currentNode) {
                HTDDecompositionPtr decomposition = app.getDecomposition();
            ...
            bool QSatCNFEDMSolver2::isUnsat(const Computation & c) {
                if (c.isLeaf()) {
            ...
            RESULT QSatCNFEDMSolver2::decide(const Computation & c) {
                Cudd manager = app.getBDDManager().getManager();
            ...
            BDD QSatCNFEDMSolver2::solutions(const Computation& c) {
                NSFManager& nsfManager = app.getNSFManager();
                std::vector<BDD> cubesAtlevels;
                for (unsigned int i = 0; i < app.getInputInstance()->quantifierCount(); i++) {
                    cubesAtlevels.push_back(app.getBDDManager().getManager().bddOne());
